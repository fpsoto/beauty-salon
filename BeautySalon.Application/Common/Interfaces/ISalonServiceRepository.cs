...
public interface ISalonServiceRepository : IRepository<SalonService>
{
    ...
    // Scoped per category (not global) - two different categories can legitimately have a
    // same-named service; case-insensitive within that scope.
    Task<SalonService?> GetByNameInCategoryAsync(string name, Guid categoryId, CancellationToken cancellationToken = default);
